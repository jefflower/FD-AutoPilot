        ...
        try {
            ...
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                ...
                if (jwtUtil.validateToken(token)) {
                    // 一次性解析 token，避免多次 parseToken 的重复开销
                    JwtUtil.ParsedToken parsed = jwtUtil.parseTokenOnce(token);

                    // 检查 token 是否已被撤销（黑名单检查）
                    if (parsed.jti() != null && tokenBlacklistService.isBlacklisted(parsed.jti())) {
                        // Token 已被撤销，跳过认证
                    ...
                    for (String role : parsed.roles()) {
                        authorities.add(new SimpleGrantedAuthority("ROLE_" + role));
                    }

                    // 从缓存加载用户权限，生成权限 GrantedAuthority（PERM_ 前缀）
                    Set<String> permissions = permissionCacheService.getUserPermissions(parsed.userId());
                    ...
                    UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
                            parsed.username(),
                            null, authorities);
                    authentication.setDetails(parsed.userId());
