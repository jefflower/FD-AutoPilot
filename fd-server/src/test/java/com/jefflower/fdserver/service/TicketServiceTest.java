...
class TicketServiceTest {
    ...
    class SubmitTranslation {
        ...
        @Test
        @DisplayName("首次翻译 → 保存翻译、状态变为 PENDING_REPLY、发送 MQ")
        void firstTranslation_savesAndTransitions() {
            ...
            verify(mqPublisherService).sendReplyTask(sampleTicket);
        }
        ...
        @Test
        @DisplayName("工单已是 PENDING_REPLY → 不重复发送 MQ")
        void alreadyPendingReply_noMqSent() {
            ...
            verify(mqPublisherService, never()).sendReplyTask(any());
        }
    ...
    class SubmitReply {
        ...
        @Test
        @DisplayName("提交回复 → 删除旧回复、保存新回复、状态变为 PENDING_AUDIT")
        void submitsReplyAndTransitions() {
            ...
            verify(mqPublisherService).sendAuditTask(sampleTicket);
            verify(weChatWorkNotifyService).notifyMqReplyCompleted(sampleTicket);
        }
    ...
    class SubmitAudit {
        ...
        void pass_autoReplyDisabled_approved() {
            ...
            verify(weChatWorkNotifyService).notifyAuditPass(sampleTicket);
        }
        ...
        @Test
        @DisplayName("审核驳回 → 状态回退 PENDING_REPLY，保存驳回意见，发送 MQ 重新回复")
        void reject_rollsBackToPendingReply() {
            ...
            verify(mqPublisherService).sendReplyTask(sampleTicket);
            verify(weChatWorkNotifyService).notifyAuditReject(sampleTicket, "回复质量不佳");
        }
    ...
    class PushApprovedReply {
        ...
        void pushesSuccessfully() {
            ...
            verify(weChatWorkNotifyService).notifyReplyPushed(sampleTicket);
        }
    ...
    class TriggerAiTranslation {
        ...
        @Test
        @DisplayName("触发翻译 → 状态变为 TRANSLATING，发送 MQ")
        void triggersTranslation() {
            ...
            verify(mqPublisherService).sendTranslationTask(sampleTicket);
        }
    }
    ...
    class TriggerAiReply {
        ...
        @Test
        @DisplayName("有翻译 → 状态变为 REPLYING，发送 MQ")
        void triggersReply() {
            when(ticketRepository.findById(1L)).thenReturn(Optional.of(sampleTicket));
