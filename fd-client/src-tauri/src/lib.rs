...
// =========== 以下全部仅在 Tauri 桌面模式下编译 ===========
#[cfg(feature = "tauri-app")]
mod tauri_app {
    use crate::ai::{GeminiClient, TauriLogger};
    ...
    #[tauri::command]
    pub async fn translate_ticket_direct_cmd(app: AppHandle, ticket: crate::models::Ticket, target_lang: String, system_prompt: Option<String>) -> Result<crate::models::Ticket, String> {
        let logger = TauriLogger { app: &app };
        GeminiClient::translate_ticket(&logger, &ticket, &target_lang, system_prompt.as_deref()).await
    }

    #[tauri::command]
    pub async fn execute_gemini_cmd(app: AppHandle, prompt: String, models: Vec<String>) -> Result<String, String> {
        let logger = TauriLogger { app: &app };
    ...
    pub async fn sync_translate_reply_cmd(
        ...
        let logger = TauriLogger { app: &app };
        GeminiClient::sync_translate_reply(&logger, &source_text, &reference_text, &direction, &target_lang).await
    }
    ...
    #[tauri::command]
    pub async fn open_shadow_window(app: AppHandle, label: String, url: String) -> Result<(), String> {
    ...
    #[tauri::command]
    pub async fn execute_shadow_js(app: AppHandle, label: String, script: String) -> Result<(), String> {
    ...
    #[tauri::command]
    pub async fn toggle_shadow_window(app: AppHandle, label: String, visible: bool) -> Result<(), String> {
    ...
    #[tauri::command]
    pub async fn get_shadow_window_visibility(app: AppHandle, label: String) -> Result<bool, String> {
        if let Some(window) = app.get_webview_window(&label) {
    ...
    #[tauri::command]
    pub async fn open_notebook_window(app: AppHandle, notebook_id: String, notebook_url: Option<String>) -> Result<(), String> {
    ...
    #[tauri::command]
    pub async fn forward_shadow_event(app: AppHandle, event: String, payload: String) -> Result<(), String> {
    ...
    #[tauri::command]
    pub async fn toggle_notebook_window(app: AppHandle, visible: bool, notebook_id: Option<String>, notebook_url: Option<String>) -> Result<(), String> {
        if let Some(window) = app.get_webview_window("notebook_shadow") {
            if visible {
                window.show().map_err(|e| e.to_string())?;
                window.set_focus().map_err(|e| e.to_string())?;
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            // 启动带 Shadow Agent 支持的 bridge server
            bridge_server::start_bridge_server_with_app(app.handle().clone());
            Ok(())
        })
